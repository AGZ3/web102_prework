...
function filterUnfundedOnly() {
    ...
    let listOfUnfundedGoal = GAMES_JSON.filter ( (game) => {
        return game.goal > game.pledged;
});

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(listOfUnfundedGoal);
}
...
function filterFundedOnly() {
    ...
    let listOfFundedGoal = GAMES_JSON.filter ( (game) => {
        return game.pledged > game.goal;
});

    // use the function we previously created to add unfunded games to the DOM
    addGamesToPage(listOfFundedGoal);
}
...
const displayStr = `
    ...
    Currently, ${totalUnfundedGames} game${totalUnfundedGames === 1 ? '' : 's'} remain${totalUnfundedGames === 1 ? 's' : ''} unfunded. 
...
// Bonus Feature: Fund This Game Button
// Skils used: create element, add to the DOM, append child, manipulate class, style.css change
function addFundButton(gameCard) {
    // Create button element
    const fundButton = document.createElement('button');
    fundButton.classList.add('fund-button');
    fundButton.textContent = 'Fund this game';

    // Append the button to game card
